...
#' @param purged If TRUE, loads the purged version of the database
#' @param lock If TRUE, gets a lock on the database (*** to be implemented)
#' @keywords internal


load_database <- function(database, purged = FALSE, lock = TRUE) {
   
   
   f <- file.path(the$dbdir, ifelse(purged, 'purged', ''), 
                  paste0(database, ifelse(purged, '_purged', ''), '.RDS'))
   
   if(is.null(the[[database]]))                                            # if we don't have the database,
      if(!file.exists(f)) {
         if(purged)
            stop('There are no purged items in this database')
         else
