   ...
   switch(database,
          'fdb' = {
             the$fdb <- data.frame(
                ...
                gpu = character(),                    # GPU(s) used (name and count; NA for non-GPU models)
                gpu_pct = double(),                   # percent GPU utilization (NA for non-GPU models)
                gpu_mem = double(),                   # GPU memory used in GB (NA for non-GPU models)
