...
config <- list(
   ...
   n_channels = 8,    # RGB + NIR + RedEdge + NDVI + NDRE + DEM
   target_classes = c(3, 4, 5, 6),  # Trans1, Trans2, Trans3, High1
   class_mapping = c("3" = 0, "4" = 1, "5" = 2, "6" = 3),  # Remap for U-Net
   val_fraction = 0.2,
   seed = 42
)
...
calc_ndre <- function(nir, red_edge) {
   (nir - red_edge) / (nir + red_edge + 1e-8)
}

# Helper: Normalize raster to 0-1
normalize_band <- function(r, method = "percentile", lower = 0.02, upper = 0.98) {
...
build_input_stack <- function(portable_name, ortho_dir, ortho_lookup, dem_lookup) {
   ...
   if (!is.null(nir)) {
      ...
      red_edge_norm <- normalize_band(red_edge)
      
      # Calculate indices
      ndvi <- calc_ndvi(nir, red)
      ndre <- calc_ndre(nir, red_edge)
...
#' @param patch_size Size of patches in pixels (e.g., 256)
...
extract_training_patches <- function(input_stack, transects, patch_size = 256, 
                                     overlap = 0.5, target_classes = c(3,4,5,6),
                                     class_mapping = c("3"=0, "4"=1, "5"=2, "6"=3)) {
   
   # Filter to target classes
   transects <- transects %>% 
      filter(Subclass %in% target_classes)
   ...
   # Initialize arrays
   patches <- array(NA, dim = c(n_patches, patch_size, patch_size, nlyr(input_stack)))
   labels <- array(NA, dim = c(n_patches, patch_size, patch_size))
   masks <- array(0, dim = c(n_patches, patch_size, patch_size))
   ...
   for (i in 1:n_patches) {
      ...
      if (actual_h < patch_size || actual_w < patch_size) {
         # Pad with zeros
         padded <- array(0, dim = c(patch_size, patch_size, nlyr(input_stack)))
      ...
      if (nrow(transects_patch) > 0) {
         ...
         template <- rast(patch_ext, nrows = patch_size, ncols = patch_size, 
                          crs = crs(input_stack))
...
#' 
#' @param patch_data List from extract_training_patches
#' @param transects Original sf transects object
#' @param val_fraction Fraction for validation (e.g., 0.2)
#' @param seed Random seed
#' @return List with train and val patch indices
spatial_train_val_split <- function(patch_data, transects, val_fraction = 0.2, seed = 42) {
   ...
   n_val <- ceiling(n_transects * val_fraction)
...
#' 
#' @param patch_data List from extract_training_patches
#' @param split_indices List from spatial_train_val_split
#' @param output_dir Directory to save numpy files
#' @param site_name Name for files (e.g., "site1")
export_to_numpy <- function(patch_data, split_indices, output_dir, site_name) {
...
prepare_site_data <- function() {
   ...
   input_stack <- build_input_stack(selected_ortho, ortho_dir, 
   ...
   split_indices <- spatial_train_val_split(
