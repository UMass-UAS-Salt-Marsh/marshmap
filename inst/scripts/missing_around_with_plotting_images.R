...
library(terra)
x <- rast('/work/pi_bcompton_umass_edu/marsh_mapping/data/oth/flights/02Aug19_OTH_Low_Mica_Ortho.tif')
plotRGB(x, 3, 2, 1, stretch = 'lin')


# for single-band images like SWIR and DEM:
x <- rast('/work/pi_bcompton_umass_edu/marsh_mapping/data/red/flights/a16Jun22_RR_High_SWIR_Ortho.tif')
plot(x, col = map.pal('bcyr'), legend = FALSE)

x <- rast('/work/pi_bcompton_umass_edu/marsh_mapping/data/red/flights/26May2022_RED_Low_HesaiRGB_DEM.tif')
plot(x, col = map.pal('bcyr'), legend = FALSE)
...
swir <- rast('/work/pi_bcompton_umass_edu/marsh_mapping/data/oth/flights/08Sep22_OTH_High_SWIR_Ortho.tif')
